...
namespace Revamp.IO.OpenXML
{
    public class _GetExcelInfo
    {
        public string GetCell(Int32 x, Int32 y)
        {
            return GetXCellViaNumber(x) + y;
        }

        public string[] getXYfromCell(string cell)
        {
            // Get the Excel column name
            string colName = Regex.Split(cell, @"\d")[0].ToUpper();
            // Get the Excel row number
            long y = 0;
            long.TryParse(Regex.Match(cell, @"\d+").Value, out y);
