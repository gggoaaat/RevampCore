...
namespace Revamp.IO.Helpers.Helpers
{
    public class IOHelper
    {
        ...
        public string GetContentTypeViaExt(string extension)
        {
        ...
        public ViewObjectPropFile GetFileByID(IConnectToDB _Connect, Guid? FileID)
        {
            ...
            Filters.Add(new DynamicModels.RootReportFilter { FilterName = "OBJ_PROP_SETS_DAT_FILE_ID_", DBType = SqlDbType.UniqueIdentifier, ParamValue = FileID });

            DataTable _DT = DynamicOutput.DynoProcSearch(_Connect, "Custom Query", "SP_S_VW__OBJ_PROP_SETS_DAT_FILE_SEARCH",
                new DataTableDotNetModelMetaData { columns = "DT_CREATED, CONTENT_TYPE, FILE_NAME", length = -1, order = "1 asc", start = 0, verify = "T" },
                Filters);
        ...
        public DataTable GetFileIDSByPropID(IConnectToDB _Connect, string _id)
        {
            _DynamicOutputProcedures DynamicOutput = new _DynamicOutputProcedures();

            List<DynamicModels.RootReportFilter> Filters = new List<DynamicModels.RootReportFilter>();

            Filters.Add(new DynamicModels.RootReportFilter { FilterName = "OBJ_PROP_SETS_ID_", DBType = SqlDbType.BigInt, ParamValue = _id });

            DataTable TempDataTable = DynamicOutput.DynoProcSearch(_Connect, "Custom Query", "SP_S_VW__OBJ_PROP_FILE_SEARCH",
                new DataTableDotNetModelMetaData { columns = "FILES_ID", length = -1, order = "1 asc", start = 0, verify = "T" },
                Filters);
        ...
        public string getFileFromLocalPath(string localPath)
        {
            ...
            ////            base64 = new char[fileContent.Length / sizeof(char)];
        ...
        {
            byte[] bytes = new byte[file.Length * sizeof(char)];
            System.Buffer.BlockCopy(file.ToCharArray(), 0, bytes, 0, bytes.Length);
            return bytes;
        }
        ...
        public string SendEmail(string toAddress, string subject, string messageBody)
        { //// Allow multiple "To" addresses to be separated by a semi-colon
            ...
            try
            {
                ...
                messageBody = "New message received: <br/><br/>" + messageBody;
        ...
        public SmtpClient SMTPConfigSettings(SmtpClient SmtpServerConfig, SMTPModel _configSettings)
        {
            ...
            catch { SMTPServerConfigUsername = "true"; }
            ...
            catch { SMTPServerConfigPassword = "true"; }
