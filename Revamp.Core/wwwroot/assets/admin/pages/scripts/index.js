            ...
            $('#dashboard-report-range').daterangepicker({
                ...
                function (start, end) {
                    ...
                    var dates = {
                        ...
                        app_id: $(".main-content .header").attr("data-app")
                    }
                    var app;
                    ...
                    var response = $.ajax({
                        ...
                        success: function (data) {
                            ...
                            //var app = response.responseText;
                            // var count = app.ActivityCounts.length;

                            var oneDay = 24 * 60 * 60 * 1000; // hours*minutes*seconds*milliseconds
                            var firstDate = new Date($(".daterangepicker_start_input input").val())
                            var secondDate = new Date($(".daterangepicker_end_input input").val())

                            var diffDays = Math.round(Math.abs((firstDate.getTime() - secondDate.getTime()) / (oneDay)));

                            for (var i = 0; i <= diffDays; i++) {
                                var currentLoopDate = new Date($(".daterangepicker_start_input input").val());
                                currentLoopDate.setDate(firstDate.getDate() + i);
                                //var dateString = (currentLoopDate.getUTCMonth() + 1) + "/" + currentLoopDate.getDate() + "/" + currentLoopDate.getYear().toString().substring(1, 3);
                                for (var a = 0; a < app.ActivityCounts.length; a++) {
                                    var date = new Date(parseInt(app.ActivityCounts[a].DT_created.substr(6)));

                                    if (currentLoopDate.getDate() == date.getDate()) {   //if activty day from DB does not equal this day in the loop then put this 
                                        activities[i] = [currentLoopDate.getDate(), app.ActivityCounts[a].count];
                                        break;
                                    }
                                    else {   //else add the date from the data base in this spot
                                        activities[i] = [currentLoopDate.getDate(), 0];
                                    }
                                }
                            }
                        }
