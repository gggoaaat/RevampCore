var main = {
    options: {
        mainDiv: "main-column-flow",
        optionsDiv: "element-options",
        leftDiv: "left-column-flow"
    },
    cache: null,
    addStep: function (e) {
    ...
    showOptions: function (c) {
        ...
        if (c.attr("class") != "row") {
            ...
            f = c.children("label");
            d += '<input type="text" id="form-options-label"  value="' + f.html().replace('<div class="rqrd">*</div>', "").replace('<div class="rqrd" style="">*</div>', "") + '"/>';
            d += '<label>Name <a class="form-tip" href="#" original-title="The name of the form field which you use at the server end to access the data submitted to the server.">(?)</a></label>';
        }
    ...
    step: function () {
        var a = '<label class="field" for="{name}">{label}</label><span class="step" id="{name}" data="{validate:{required:false,messages:{},conditional:false}}"><div class="flow-step-text"><h3>if:</h3></div> <div class="flow-step-if " > <ul class="btn-bar "> </ul> </div><br/><div class="flow-step-text"> <h3>then:</h3> </div><div class="flow-step-then"> <ul class="btn-bar "></ul> </div></span>';
        return main.stepHTML(a, "step")
    },
    actor: function () {
        var a = '<li class="stepButton currentButton"><div id="{name}" class="actor" ><a data-toggle="popover" data-placement="bottom" data-content="#" href="#" class="btn green" ><i class="icon-user"></i> <span class="btn-text">Actor</span> <i class=" icon-remove"></i></a>	</div></li>';
        return main.commonHTML(a, "actor")
    },
    action: function () {
        var a = '<li class="stepButton currentButton"><div id="{name}" class="action" ><a data-toggle="popover" data-placement="bottom" data-content="#" href="#" class="btn blue dropdown-toggle"><i class="icon-cog"></i> <span class="btn-text">Action</span> <i class=" icon-remove"></i></a></div></li>';;
        return main.commonHTML(a, "action")
    },
    and: function () {
        var a = '<li class="stepButton currentButton"><div id="{name}" class="and"  ><a data-toggle="popover" data-placement="bottom" data-content="#" href="#" class="btn purple dropdown-toggle"><i class="icon-info-sign"></i> <span class="btn-text">And</span> <i class=" icon-remove"></i></a></div></li>';
        return main.commonHTML(a, "and")
    },
    or: function () {
        var a = '<li class="stepButton currentButton"><div id="{name}" class="or" ><a data-toggle="popover" data-placement="bottom" data-content="#" href="#" class="btn purple dropdown-toggle"><i class="icon-info-sign"></i> <span class="btn-text">Or</span> <i class=" icon-remove"></i></a></div></li>';
        ...
        switch (e) {
            case "dropDown":
                $(".form-options-more").each(function () {
                    //alert('b ' + $(this).val());
                    //alert('c ' + $('#form-options-name-select').val());
                    if ($(this).val() == $('#form-options-name-select').val()) {
                        d += '<option value="' + $('#form-options-name-value').val() + '">' + $(this).val() + "</option>";
                    }
                    else {
            ...
            case "radioButton":
                ...
                $(".form-options-more").each(function () {
                    ...
                    d += '<label class="option" for="' + g + '">' + $(this).val() + '<input class="radio" type="radio" name="' + c + '" id="' + g + '" value="' + $(this).val() + '" />' + '' + "</label>"
                });
